...
import { getAsiaDate, getNextSpawn } from './utils/time';
import './App.css';
...
function getServerDate(now, offsetHours, manualTime) {
  const asiaDate = getAsiaDate(now, offsetHours);
  ...
  const manualDate = new Date(asiaDate);
...
function App() {
  ...
  const serverDate = useMemo(
    () => getServerDate(now, offsetHours, manualTime),
    [manualTime, now, offsetHours],
  );
  ...
  return (
    <ThemeProvider theme={theme}>
      <CssBaseline />
      <AppHeader classes={classes} serverDate={serverDate} />
